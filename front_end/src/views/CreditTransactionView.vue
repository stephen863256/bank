<template>
    <el-card class="box-card">
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="creditAccount" label="账户号">
            <template v-slot:default="scope">
                <el-input v-if="scope.row.editing" v-model="scope.row.creditAccount" />
                <span v-else>{{scope.row.creditAccount}}</span>
            </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog title="搜索还款记录" v-model="showSearchDialog">
        <el-form :model="searchForm">
            <el-form-item label="账户号" label-width="100px">
                <el-input v-model="searchForm.creditAccount" autocomplete="off" />
            </el-form-item>
        </el-form>
        <template #footer>
...
export default {
  setup() {
    ...
    const loadData = async () => {
      try {
        const response = await request.post('/api/credittransactions/get/', searchForm, {
          params: {
